...
namespace flipbits.SummonMenu
{
    ...
    public class ODMenuCtrl : UdonSharpBehaviour
    {
        ...
        [SerializeField] Vector3 menuDesktopScale;
        ...
        public void SetMenuOrigin(Transform origin)
        {
            if (origin == null)
                return;

            menuOriginPos = origin.position;
            menuOriginRot = origin.rotation;

            // Move the menu to the new origin position if it's currently not on wrist.
            if (!isMenuActive)
            {
                menu.transform.position = menuOriginPos;
                menu.transform.rotation = menuOriginRot;
        ...
        void ActivateMenuOnDesktop()
        {
            ...
            // Set the menu scale
            menu.transform.localScale = menuDesktopScale;
        ...
        private void Update()
        {
            if (isVr)
            {
                ...
                bool leftStickClick = Input.GetButton("Oculus_CrossPlatform_PrimaryThumbstick");
            ...
            else
            {
                ...
                // Calculate distance from player and deactivate if player gets too far
                if (isMenuActive)
                {
                    Vector3 ppos = Networking.LocalPlayer.GetPosition();

                    Vector3 sub = ppos - menu.transform.position;
